...
| 数据模型 | `tb_conversation.thread_id/checkpoint_status`、`tb_session_audit`、`lg_checkpoint` schema | PR-A |
...
| 启用粒度 | **所有 agent 类型应用默认开启**；通过 `AgentRunRequest.use_checkpoint` 控制单次调用是否走 checkpoint（API 直调可关） |
...
```
AgentRunRequest.use_checkpoint = False（API 直调一次性调用）:
  messages = 全量历史（API 调用方自己拼上下文）

use_checkpoint=True + checkpoint 命中:
  messages = [本轮新一句]                     ← LangGraph 从 saver 恢复历史

use_checkpoint=True + checkpoint 缺失:
  if prior_count <= 1:
      messages = [本轮新一句]                 ← 首次调用
  else:
